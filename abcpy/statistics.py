...
class WoodRickerStatistics(Statistics):
    """
    This class implements the statistics function from the Statistics protocol. This
    extracts the statistics following Wood [1] from the timeseries generated by the
    Ricker model of animal population size.

    [1] S. N. Wood. Statistical inference for noisy nonlinear ecological
    dynamic systems. Nature, 466(7310):1102–1104, Aug. 2010.
    """

    def __init__(self, obs_data, degree=2, cross=True):
        """

        Parameters
        ----------
        obs_data: Python list
    ...
    def statistics(self, data):
        num_element = len(data)
        result = np.zeros(shape=(num_element, 13))
        # Compute statistics
        for ind_element in range(0, num_element):
            data_ind_element = np.array(data[ind_element])
            # Mean
            result[ind_element, 0] = np.mean(data_ind_element)
            # Number of zero observations
            result[ind_element, 1] = data_ind_element[np.where(data_ind_element == 0)].shape[0]
            # Auto covariances lag 1 up to 5
            result[ind_element, 2] = self._auto_covariance(data_ind_element, lag=1)
            result[ind_element, 3] = self._auto_covariance(data_ind_element, lag=2)
            result[ind_element, 4] = self._auto_covariance(data_ind_element, lag=3)
            result[ind_element, 5] = self._auto_covariance(data_ind_element, lag=4)
            result[ind_element, 6] = self._auto_covariance(data_ind_element, lag=5)
            # Least squares regression coefficients on observed data
            delta_y = np.add(data_ind_element[1:], np.negative(data_ind_element[:-1]))
            delta_yobs = np.add(self.obs[1:], np.negative(self.obs[:-1]))
            result[ind_element, 7:11] = self._least_squares(delta_yobs, delta_y)
            # Least squares regression coefficients for model
            yy = np.power(abs(data_ind_element[1:]), 0.3)
            xx = abs(data_ind_element[:-1])
            X = np.column_stack((np.power(xx, 0.3), np.power(xx, 0.6)))
            beta = np.linalg.lstsq(X, yy)[0]
            result[ind_element, 11:] = beta
    ...
    def _least_squares(self, x, Y, n_poly=3):
        """
        Calculate the least squares regression estimate of X for targets Y.
        ...
        # Create design matrix, include bias term.
        X = np.ones((x.shape[0],))
        for ind in range(1, n_poly + 1):
            X = np.column_stack((X, np.power(x, ind)))
